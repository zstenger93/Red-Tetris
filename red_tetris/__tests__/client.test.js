const {
  ...
  it('should correctly color player 1 next piece cells', () => {
	const data = {
		player1NextPiece: '4KA',
	};
	colorTheNextPiece(data);
	expect(document.getElementById).toHaveBeenCalledWith('tetrisDashBoard1_KA');
	expect(mockElement.style.backgroundColor).toBe('#9E237F');
	expect(document.getElementById).toHaveBeenCalledWith('tetrisDashBoard1_KA');
	expect(document.getElementById).toHaveBeenCalledWith('tetrisDashBoard1_KA');
  });

  it('should correctly color next piece cells for both players', () => {
	const data = {
		player1NextPiece: '4KA',
		player2NextPiece: '4KA',
	};
	colorTheNextPiece(data);
	...
	expect(document.getElementById).toHaveBeenCalledWith('tetrisDashBoard2_KA');
	expect(mockElement.style.backgroundColor).toBe('#9E237F');
  });

  it('should not throw error if player1NextPiece is null', () => {
	const data = {
		player1NextPiece: null,
	};
	expect(() => colorTheNextPiece(data)).not.toThrow();
  });

  it('should not throw error if player2NextPiece is null', () => {
	const data = {
		player2NextPiece: null,
	};
	expect(() => colorTheNextPiece(data)).not.toThrow();
  });
});
